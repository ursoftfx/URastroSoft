...
export const OnePageReport = ({ result }: Props) => {
  ...
  return (
    ...
    <div className="a4-sheet print-area" style={{ width: "210mm", height: "297mm", padding: "8mm 10mm", margin: "auto", background: "white", color: "#000", fontFamily: "'Latha','Tahoma',sans-serif", boxSizing: "border-box", overflow: "hidden" }}>
      ...
      {/* === 120-year Dasha-Bhukti static A4 pages (one per Mahadasha) === */}
      {result.dashaTree.map((maha, mi) => (
        <div key={mi} className="a4-sheet print-area" style={{ width: "210mm", minHeight: "297mm", padding: "10mm 12mm", margin: "8mm auto", background: "white", color: "#000", fontFamily: "'Latha','Tahoma',sans-serif", boxSizing: "border-box", pageBreakBefore: "always" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "2px solid #7a1a2b", paddingBottom: 6, marginBottom: 8 }}>
            <div>
              <div style={{ fontSize: 18, fontWeight: 800, color: "#7a1a2b" }}>120 ஆண்டு தசா புத்தி</div>
              <div style={{ fontSize: 11, color: "#555" }}>{i.name} • {fmtDate(birthDate)} {fmtTimeStr(i.hour, i.minute)}</div>
            </div>
            <div style={{ textAlign: "right", fontSize: 10, color: "#555" }}>பக்கம் {mi + 1} / {result.dashaTree.length}</div>
          </div>

          <div style={{ background: "#fbe9d0", border: "1px solid #c9a050", padding: "4px 8px", fontSize: 13, fontWeight: 700, textAlign: "center" }}>
            {maha.lord} மகா தசை &nbsp;•&nbsp; {fmtDateLong(maha.startDate)} → {fmtDateLong(maha.endDate)}
          </div>

          <table style={{ width: "100%", fontSize: 10, borderCollapse: "collapse", border: "1px solid #c9a050", marginTop: 6 }}>
            <thead>
              <tr style={{ background: "#fff8ee" }}>
            ...
            <tbody>
              {(maha.children || []).flatMap((bhukti, bi) => {
                ...
                if (antaras.length === 0) {
                  rows.push(
                    <tr key={`${bi}-empty`}>
                      <td style={{ border: "1px solid #c9a050", padding: 3, fontWeight: 700 }}>{maha.lord}/{bhukti.lord}</td>
                      <td style={{ border: "1px solid #c9a050", padding: 3 }}>{fmtDateLong(bhukti.startDate)}</td>
                      <td style={{ border: "1px solid #c9a050", padding: 3 }}>{fmtDateLong(bhukti.endDate)}</td>
                      <td colSpan={3} style={{ border: "1px solid #c9a050", padding: 3, textAlign: "center", color: "#888" }}>—</td>
                    </tr>
                  );
                } else {
                  antaras.forEach((ant, ai) => {
                    rows.push(
                      <tr key={`${bi}-${ai}`}>
                        {ai === 0 && (
                          <>
                            <td rowSpan={span} style={{ border: "1px solid #c9a050", padding: 3, fontWeight: 700, background: "#fff8ee", verticalAlign: "top" }}>{maha.lord}/{bhukti.lord}</td>
                            <td rowSpan={span} style={{ border: "1px solid #c9a050", padding: 3, verticalAlign: "top" }}>{fmtDateLong(bhukti.startDate)}</td>
                            <td rowSpan={span} style={{ border: "1px solid #c9a050", padding: 3, verticalAlign: "top" }}>{fmtDateLong(bhukti.endDate)}</td>
                          </>
                        )}
                        <td style={{ border: "1px solid #c9a050", padding: 3 }}>{ant.lord}</td>
                        <td style={{ border: "1px solid #c9a050", padding: 3 }}>{fmtDateLong(ant.startDate)}</td>
                        <td style={{ border: "1px solid #c9a050", padding: 3 }}>{fmtDateLong(ant.endDate)}</td>
                      </tr>
                    );
                  });
                }
